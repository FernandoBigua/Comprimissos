document.addEventListener("DOMContentLoaded", () => {
  ...
  const filtroResponsabilidadeSelect = document.getElementById(
    "filtroResponsabilidade",
  );
  const compromissosTableBody = document.querySelector(
    "#compromissosTable tbody",
  );
  ...
  async function carregarDados() {
    try {
      ...
      compromissosTableBody.innerHTML =
        '<tr><td colspan="5">Erro ao carregar os dados.</td></tr>';
    }
  ...
  function popularFiltros() {
    ...
    const compromissosUnicos = [
      ...new Set(todosOsDados.map((item) => item.COMPROMISSO)),
    ];
  ...
  function renderizarTabela() {
    if (filtroResponsabilidadeSelect.value === "") {
      compromissosTableBody.innerHTML =
        '<tr><td colspan="4">Selecione uma responsabilidade para ver os resultados.</td></tr>';
      return;
    }
    ...
    if (dadosFiltrados.length === 0) {
      compromissosTableBody.innerHTML =
        '<tr><td colspan="4">Nenhum compromisso encontrado com os filtros selecionados.</td></tr>';
      return;
    }

    // Agrupa os dados por COMPROMISSO
    const gruposPorCompromisso = {};
    dadosFiltrados.forEach((item) => {
      if (!gruposPorCompromisso[item.COMPROMISSO]) {
        gruposPorCompromisso[item.COMPROMISSO] = [];
      }
      gruposPorCompromisso[item.COMPROMISSO].push(item);
    });

    // Renderiza cada grupo de compromisso
    Object.entries(gruposPorCompromisso).forEach(([compromisso, itens]) => {
      // Adiciona linha de cabeçalho do compromisso
      const rowCompromisso = compromissosTableBody.insertRow();
      rowCompromisso.classList.add("compromisso-header");
      const cellCompromisso = rowCompromisso.insertCell();
      cellCompromisso.colSpan = 4;
      cellCompromisso.textContent = compromisso;

      // Adiciona os itens deste compromisso
      itens.forEach((item) => {
    ...
    document.getElementById("infoResponsabilidade").textContent =
      responsabilidadeSelecionada
        ? `Responsabilidade: ${responsabilidadeSelecionada}`
        : "";
  }
